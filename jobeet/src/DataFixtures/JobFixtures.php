...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager) : void
    {
        ...
        $jobExpired = new Job();
        $jobExpired->setCategory($manager->merge($this->getReference('category-design')));
        $jobExpired->setType('part-time');
        $jobExpired->setCompany('Extreme Sensio 2 Expired');
        $jobExpired->setLogo('extreme-sensio.gif');
        $jobExpired->setUrl('http://www.extreme-sensio.com/');
        $jobExpired->setPosition('Web Designer');
        $jobExpired->setLocation('Paris, France');
        $jobExpired->setDescription('Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in.');
        ...
        $jobExpired->setToken('job_extreme_sensio_expired');
        $jobExpired->setEmail('job@example.com');
        $jobExpired->setExpiresAt(new \DateTime('-50 days'));
